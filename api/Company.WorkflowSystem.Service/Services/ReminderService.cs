    ...
    public class ReminderService : BaseService
    {
        ...
        async Task DealExpiryDateReminders()
        {
            ...
            foreach (var day in days)
            {
                ...
                var deals = await _repo.ProjectedListBuilder((Deal d) => new { d.Id, d.DealNumber })
                    .WhereEntity(d => !d.CurrentDealWorkflowStatus.Finalized)
                    .WhereEntity(d => d.ExpiryDate.HasValue && d.ExpiryDate >= dateMin && d.ExpiryDate <= dateMax)
                    .ExecuteAsync();
        ...
        async Task DealNotesReminders()
        {
            ...
            var notes = await _repo.ProjectedListBuilder((DealNote n) => new
            {
                ...
                .WhereEntity(n => !n.Deal.CurrentDealWorkflowStatus.Finalized && n.ReminderDateTime.HasValue && n.ReminderType.HasValue)
                .WhereEntity(n => n.ReminderDateTime >= todayStart && n.ReminderDateTime <= todayEnd)
                .ExecuteAsync();
