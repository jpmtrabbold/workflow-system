...
namespace Company.DealSystem.Application.Services
{
    public class DealWorkflowService : BaseService
    {
        ...
        public async Task<DealAssignInfoResponse> DealAssignInfo(int dealId)
        {
            ...
            {
                AssignedToSelf = Deal.AssignedToSpecificUser(d, userId),
                AssignedToUsersRole = Deal.AssignedToUsersRole(d, userId),
        ...
        async Task<WorkflowStatusReadDto> GetCurrentWorkflowStatusConfig(int workflowSetId, int? currentDealWorkflowStatusId = null)
        {
            ...
            var workflowBuilder = _repo.ProjectedListBuilder(WorkflowStatusReadDto.ProjectionFromEntity())
                .Where(ws => ws.WorkflowSetId == workflowSetId)
                .OrderBy(ws => ws.Order);
            ...
            {
                currentWorkflowStatusId = (await _repo.ProjectedList<DealWorkflowStatus, int?>(dws => dws.WorkflowStatusId, dws => dws.Id == currentDealWorkflowStatusId)).FirstOrDefault();
            ...
            {
                var additionalActionsWithoutSource = await _repo.ProjectedList(WorkflowActionReadDto.ProjectionFromEntity(),
                wa => wa.Active
        ...
        {
            var maxItems = _configuration.GetValue<int>("MaxDealItemsPerEmail");

            var dealItems = (await _repo.ProjectedListBuilder(DealItemDto.ProjectionFromEntity)
                .Where(t => t.DealId == entity.Id)
            ...
            for (int i = 0; i < dealItems.Count; i++)
            {
                if (i == maxItems)
                {
                    var phrase = $"These are the first {maxItems} dealItems of the deal. " +
                        $"Please open the deal on DealSystem or refer to Middle Office " +
            ...
            if (Updatable.IsUpdated(deal.OngoingWorkflowActionId))
            {
                ...
                if (actionId.HasValue)
                {
                    ...
                    if (workflowAction.CantBePerformedBySameUser)
                    {
                        ...
                        var otherActionName = (await _repo.ProjectedList(
                            (WorkflowAction action) => action.Name,
                            action => action.CantBePerformedBySameUser && otherWorkflowActionIds.Contains(action.Id)))
                            .FirstOrDefault();
