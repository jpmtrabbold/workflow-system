                ...
                foreach (var dealItem in dealItems)
                {
                    ...
                    if (existingDeal != null)
                    {
                        if (existingDeal.Cancelled)
                        {
                            ...
                            {
                                integrationRun.AddInfoEntry($"Trade {dealItem.SourceData.SourceId} is already in DealSystem " +
                                $"under deal number {existingDeal.DealNumber}, but it was cancelled. This deal will be reintegrated, " +
                                $"as DealSystem is configured to reintegrate cancelled deals.");
                                reintegrated = true;
                            }
                            else
                            {
                                integrationRun.AddInfoEntry($"Trade {dealItem.SourceData.SourceId} is already in DealSystem " +
                                $"under deal number {existingDeal.DealNumber}. Even though the deal is cancelled, DealSystem is " +
