    ...
    public class DealService : BaseService
    {
        ...
        async public Task<DealsListResponse> List(DealsListRequest listRequest)
        {
            ...
            var builder = _repo.ProjectedListBuilder(DealListDto.ProjectionFromEntity(userId), listRequest)
                ...
                .ConditionalOrder("dealStatusName", d => (d.CurrentDealWorkflowStatus != null ? d.CurrentDealWorkflowStatus.WorkflowStatus.Name : "No Status"))
                .ConditionalOrder("counterpartyName", d => d.Counterparty.Name)
                .ConditionalOrder("dealCategoryName", d => d.DealCategory.Name)
                .ConditionalOrder("dealTypeName", d => d.DealType.Name)
                .ConditionalOrder("assignedTo", d => Deal.AssignedToDescription(d))
                .ConditionalOrder("creationDate", d => d.CreationDate)
                .ConditionalOrder("creationUserName", d => d.CreationUser.Name)
            ...
            if (!listRequest.IncludeFinalizedDeals)
                builder.Where(d => !d.CurrentDealWorkflowStatus.Finalized);

            if (listRequest.OnlyDealsAssignedToMeOrMyRole)
                builder.Where(d => Deal.AssignedToSpecificUser(d, userId) || Deal.AssignedToUsersRole(d, userId));
            ...
            if (!string.IsNullOrWhiteSpace(str))
            {
                builder.Where(d =>
                    ...
                    || d.Counterparty.Name.Contains(str)
                    || (d.CurrentDealWorkflowStatus != null ? d.CurrentDealWorkflowStatus.WorkflowStatus.Name.Contains(str) : false)
                    || d.DealType.Name.Contains(str)
                    || d.DealCategory.Name.Contains(str)
                    || Deal.AssignedToDescription(d).Contains(str)
                    || (d.Executed ? "Yes" : "No").Contains(str)
                    || d.CreationUser.Name.Contains(str)
                    );
            }
        ...
        public async Task<(byte[] fileContent, string fileDownloadName)> GetAttachmentVersionBinaryFile(int attachmentVersionId)
        {
            ...
            return (version.FileContent, $"{version.DealNumber} - {version.FileName}.{version.FileExtension}");
        }
        ...
        async public Task<List<LookupRequest>> GetDealTypes(int dealCategoryId)
        {
            ...
            deals
                .Where(dt => dt.DealCategoriesInDealType.Any(pt => pt.DealCategoryId == dealCategoryId))
                .OrderBy(dt => dt.Name);
        ...
        async public Task<List<LookupRequest>> GetProducts(int dealCategoryId)
        {
            return await _repo.ProjectedListBuilder(LookupRequest.ProjectionFromProduct)
                .Where(dt => dt.DealCategoryId == dealCategoryId)
                .OrderBy(dt => dt.Name)
                .ExecuteAsync();
        }
        /// <summary>
        ...
        async public Task<List<DealItemDto>> GetItems(int dealId)
        {
            return await _repo.ProjectedListBuilder(DealItemDto.ProjectionFromEntity)
                .Where(e => e.DealId == dealId && !e.OriginalItemId.HasValue)
                .OrderBy(t => t.Order)
                .ExecuteAsync();
        }
        ...
        public async Task<DealDirectWorkflowActionResponse> PerformDirectWorkflowAction(DealDirectWorkflowActionRequest request)
        {
            ...
            var dealDetails = (await _repo.ProjectedList((Deal d) => new { d.CurrentDealWorkflowStatusId, d.DealNumber }, d => d.Id == request.DealId)).FirstOrDefault();
            ...
            if (!hasListener)
                return response.Error("This deal's status has changed so this action became obsolete. Please access it on DealSystem for more information.");
